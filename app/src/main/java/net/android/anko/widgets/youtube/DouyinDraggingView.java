...
public class DouyinDraggingView extends RelativeLayout implements View.OnClickListener {
    ...
    // 可拖动的view 和下方的详情View
    DouyinDispatchLayout mBackgroundView;
    View titleLayout;
    View pauseIv;
    View closeIv;
    View pauseLayout;
    ...
    public DouyinDraggingView(Context context) {
        this(context, null);
    }

    public DouyinDraggingView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public DouyinDraggingView(Context context, AttributeSet attrs, int defStyle) {
    ...
    private void init() {
        ...
        addView(LayoutInflater.from(getContext()).inflate(R.layout.douyin_dispatch, null), 0);
    ...
    private void initData() {
        ...
        if (isLandscape()) {
            ...
            MIN_RATIO_HEIGHT_NODE = 0.20f;
            MIN_RATIO_HEIGHT = 0.12f;
            if (activityFullscreen) {
                mTopOriginalHeight = DensityUtil.getScreenH(getContext());
            } else {
                mTopOriginalHeight = DensityUtil.getScreenH(getContext()) - DensityUtil.getStatusBarH(getContext());
    ...
    public void resetRangeAndSize() {
        mTopViewOriginalWidth = DensityUtil.getScreenW(getContext());
        if (isLandscape()) {
            mTopOriginalHeight = activityFullscreen ? DensityUtil.getScreenH(getContext()) : getScreenOriginH(getContext());
            MIN_RATIO_HEIGHT_NODE = 0.35f;
            MIN_RATIO_HEIGHT = 0.25f;
        } else {
                mTopOriginalHeight = activityFullscreen ? DensityUtil.getScreenH(getContext()) : getScreenOriginH(getContext());
        ...
        float height = activityFullscreen ? DensityUtil.getScreenH(getContext()) : getScreenOriginH(getContext());
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.closeLayout:
                dismissView();
                mCallback.onIconClick(IconType.CLOSE);
                break;
        }
    }

    public int getScreenOriginH(Context c) {
        return DensityUtil.getScreenH(c) - DensityUtil.getStatusBarH(c);
    }
    ...
    public boolean handleKeyDown(int keyCode) {
        if (getVisibility() == View.VISIBLE && keyCode == KeyEvent.KEYCODE_BACK && getNowStateScale() == 1f) {
            if (isPortraitToLandscape) {
                isPortraitToLandscape = false;
                mActivity.setRequestedOrientation(ActivityInfo.SCREEN_ORIENTATION_SENSOR_PORTRAIT);
                goPortraitMax();
